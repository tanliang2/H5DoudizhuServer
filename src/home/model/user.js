...
export default class extends think.model.mongo {
    ...
    async updateUserRecord(udata,roomtype) {
        ...
        await this.where({ "userData.uid": Number(udata.uid) }).update({
            ...
            $push: { 'userData.recordData.recordList.3': { time: new Date().toLocaleString(), resultList: recordData.resultList, roomId: recordData.roomId,paybackId:recordData.paybackId } }
        });
       var userdata = await this.where({ "userData.uid": Number(udata.uid) }).find();
        if(userdata.userData.recordData.recordList["3"].length > 50)
        {
            await this.where({ "userData.uid": Number(udata.uid) }).update({
                $pop: { "userData.recordData.recordList.3": -1 } 
            });
        }
    }
    //获取游戏战绩记录
