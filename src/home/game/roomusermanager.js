...
export default class roomuserManager extends think.base {
    ...
    startUp() {
        ...
        roomData.iThinkTime = this.roomModel.m_iThinkTime;
        roomData.iRobNTTime = this.roomModel.m_iRobNTTime;
    ...
    gameBegin(data) {
        ...
        this.socket.emit("gameBegin", data);
    }
    //发牌
    sendAllCard(data) {
        this.socket.emit("sendAllCard", data);
    }
    //发牌结束
    sendCardFinish(data) {
        this.socket.emit("sendCardFinish", data);
    }
    //事件监听
    clientEventHandle() {
        var _this = this;
            ...
            setTimeout(function () {
                ...
                if (!_this.clientonline) {
                    if (eventDispatcher.socketioProxy)
                        eventDispatcher.socketioProxy.removeSocket(_this.socket);
        ...
        _this.socket.on("continueGame", function () {
            ...
            if (!_this.roomModel) return;
            _this.currentState = structData.GAME_ST_READY;
            _this.roomModel.m_bUserReady[+_this.deskStation - 1] = true;
        ...
        //叫分
        _this.socket.on("ASS_CALL_SCORE", function (data) {
            if (!_this.roomModel) return;
            _this.roomModel.UserCallScore(+_this.deskStation - 1, data.iValue);
        })
        //抢地主
        _this.socket.on("ASS_ROB_NT", function (data) {
            _this.assRobNt(data);
        })
        //加棒
        _this.socket.on("ASS_ADD_DOUBLE", function (data) {
            if (_this.roomModel.m_bGameStation == structData.GS_WAIT_NEXT || _this.roomModel.m_bGameStation == structData.GS_WAIT_ARGEE || _this.roomModel.m_bGameStation == structData.GS_WAIT_SETGAME) {
                return true;
            }
            _this.roomModel.UserAddDouble(+data.bDeskStation - 1, data.iValue);
        })
        //明牌
        _this.socket.on("ASS_SHOW_CARD", function (data) {
            if(!_this.roomModel) return;
            if (_this.roomModel.m_bGameStation == structData.GS_WAIT_NEXT || _this.roomModel.m_bGameStation == structData.GS_WAIT_ARGEE || _this.roomModel.m_bGameStation == structData.GS_WAIT_SETGAME) {
                return true;
            }
            _this.roomModel.UserShowCard(+data.bDeskStation - 1, data.iValue);
        })
        //出牌
        _this.socket.on("ASS_OUT_CARD", function (data) {
            if(!_this.roomModel) return;
            if (_this.roomModel.m_bGameStation != structData.GS_PLAY_GAME) {
                return true;
            }
            _this.roomModel.UserOutCard(+_this.deskStation - 1, data.iCardList, data.iCardCount);
        })
        //托管
        _this.socket.on("ASS_AUTO", function (bAuto) {
            if(!_this.roomModel) return;
            _this.roomModel.UserSetAuto(+_this.deskStation - 1,bAuto);
        })
        //有事请求离开
        _this.socket.on("ASS_HAVE_THING", function (szMessage) {
            _this.roomModel.UserHaveThing(+_this.deskStation - 1, szMessage);
        })
        //用户同意离开
        _this.socket.on("ASS_LEFT_RESULT", function (bArgeeLeave) {
            _this.roomModel.ArgeeUserLeft(+_this.deskStation - 1,bArgeeLeave);
        })
    ...
    assRobNt(data) {
        ...
        _this.roomModel.UserRobNT(+data.byDeskStation - 1, data.iValue);
    }
    //处理玩家出牌
